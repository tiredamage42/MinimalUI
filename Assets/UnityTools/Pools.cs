...
namespace UnityTools {
    ...
    public abstract class Pool<T> where T : Component {
        protected T FindAvailableInstance (List<T> pool, Transform newParent, bool setActive) {
            for (int i = 0; i < pool.Count; i++) {
                T c = pool[i];
                
                if (!c.gameObject.activeSelf) {

                    if (c.transform.parent != newParent)
        ...
        protected T FindAvailableInstance (List<T> pool, Transform newParent, bool setActive, Vector3 position, Quaternion rotation) {
            for (int i = 0; i < pool.Count; i++) {
                T c = pool[i];
                if (!c.gameObject.activeSelf) {
    ...
    public class PrefabPool<T> : Pool<T> where T : Component {
        ...
        public T GetAvailable (T prefab, Transform newParent, bool setActive, Action<T> onCreateNew) {
            return GetAvailable(prefab, newParent, setActive, Vector3.zero, Quaternion.identity, onCreateNew);
        }
            
        public T GetAvailable (T prefab, Transform newParent, bool setActive, Vector3 position, Quaternion rotation, Action<T> onCreateNew) {
            List<T> prefabPool;
            if (pool.TryGetValue(prefab.GetInstanceID(), out prefabPool)) {
                T r = FindAvailableInstance(prefabPool, newParent, setActive, position, rotation);
    ...
    public class ComponentPool<T> : Pool<T> where T : Component {
        ...
        public T GetAvailable () {
            T r = FindAvailableInstance(pool, null, true);
